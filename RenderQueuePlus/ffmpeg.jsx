var FFMPEG = function() {
  var re = /(dpx)|(gif)|(jpg)|(targa)|(sgi)|(tiff)|(png)|(exr)/gi;
  var ffmpeg_cmd = '"[ffmpeg]" -y -f image2 -framerate [framerate] -s [width]x[height]' +
  ' -i "[basepath]' + sep + '[basename][padding].[ext]" [frameoverlay] -start_number [start]' +
  ' -frames:v [duration] [customOptions] "[basepath]' + sep + '[basename]h264.mp4"';
    ...
    getCommand: function(dataItem) {
      ...
      if (!(getSetting('ffmpeg_bin') && getSetting('ffmpeg_call'))) {
       ...
       return ffmpeg_cmd.
        ...
        replace(/\[basepath\]/g, String(dataItem.basepath)).
